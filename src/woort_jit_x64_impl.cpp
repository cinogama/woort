...
struct woort_JIT_Asmjit_x64_emitter
{
    Compiler* c;
     ...
     * never skips the mov), so a missed flush only ever forgoes optimization;
     ...
     * function, hoisting the 10-byte immediate materialization out of hot loops. */
    ...
    woort_JIT_Asmjit_x64_emitter(const woort_JIT_Asmjit_x64_emitter&) = delete;
    woort_JIT_Asmjit_x64_emitter(woort_JIT_Asmjit_x64_emitter&&) = delete;
    woort_JIT_Asmjit_x64_emitter& operator =(const woort_JIT_Asmjit_x64_emitter&) = delete;
    woort_JIT_Asmjit_x64_emitter& operator =(woort_JIT_Asmjit_x64_emitter&&) = delete;

    woort_JIT_Asmjit_x64_emitter(const woort_CodeEnv* cenv_, const woort_Bytecode** ip) noexcept
        : c(nullptr)
        , cenv(cenv_)
    ...
    ~woort_JIT_Asmjit_x64_emitter() noexcept
    {
        if (c != nullptr)
            delete c;
    }
     ...
     * the lazily-cached base register instead of a fresh 10-byte absolute
     * immediate materialization on every access. */
    Mem static_slot(woort_Opcode_Global idx)
    {
        return qword_ptr(get_static_base(),
     ...
     * UNPACKVEC. Mirrors the proven tag-decode of the CASTDYN/CASTR opcodes.
    ...
    void emit_unbox_dyn_no_check(const Gp& out, const Gp& val_in)
    {
        ...
        WOORT_JIT_CODE(jnz(L_imm));

        /* --- pointer path (low 3 bits == 0) --- */
        WOORT_JIT_CODE(test(val, val));                 /* nil (== 0)? */
        WOORT_JIT_CODE(jz(L_plain));
        {
            const Gp proxy = c->new_gp64();
            WOORT_JIT_CODE(mov(proxy, qword_ptr(val))); /* m_proxy at offset 0 */
        ...
        WOORT_JIT_CODE(bind(L_imm));
        {
            ...
            WOORT_JIT_CODE(jnz(L_real));

            WOORT_JIT_CODE(test(val, Imm(0b010)));     /* INT tag = 0b010 */
            WOORT_JIT_CODE(jnz(L_int));

            /* BOOL: out = val >> 3 (yields 0/1) */
            WOORT_JIT_CODE(mov(out, val));
            WOORT_JIT_CODE(shr(out, Imm(3)));
            WOORT_JIT_CODE(jmp(L_done));

            WOORT_JIT_CODE(bind(L_int));
            WOORT_JIT_CODE(mov(out, val));
            WOORT_JIT_CODE(sar(out, Imm(2)));
            WOORT_JIT_CODE(jmp(L_done));
            ...
            WOORT_JIT_CODE(bind(L_real));
            {
                ...
                WOORT_JIT_CODE(shr(exp_bit, Imm(62)));
                WOORT_JIT_CODE(and_(exp_bit, Imm(1)));
                WOORT_JIT_CODE(xor_(exp_bit, Imm(1)));
                WOORT_JIT_CODE(shl(exp_bit, Imm(62)));

                const Gp low62_mask = c->new_gp64();
                WOORT_JIT_CODE(mov(low62_mask, Imm(static_cast<int64_t>(0x3FFFFFFFFFFFFFFFULL))));
                WOORT_JIT_CODE(mov(out, val));
                WOORT_JIT_CODE(shr(out, Imm(1)));
                WOORT_JIT_CODE(and_(out, low62_mask));
                WOORT_JIT_CODE(or_(out, exp_bit));
                WOORT_JIT_CODE(or_(out, sign));
            }
        }
    ...
    {
        return qword_ptr(m_sb, slot * static_cast<int32_t>(sizeof(woort_Value)));
    }
    Gp load_stack_gp(woort_Opcode_Stack src)
    {
        ...
        WOORT_JIT_CODE(mov(reg, sb_slot(src)));
        return reg;
    }
    void store_stack(woort_Opcode_Stack dst, const Gp& v)
    {
        ...
        WOORT_JIT_CODE(mov(sb_slot(dst), v));
    ...
    /* A1: FP (XMM) results bypass the Gp cache; invalidate so a later
    ...
    void store_stack(woort_Opcode_Stack dst, const Vec& v)
    {
        ...
        WOORT_JIT_CODE(movq(sb_slot(dst), v));
    }
     ...
     * Equivalent to k individual `cmp m_sp, m_stack; ja` checks (the binding
...
bool woort_JIT_Backend_x64_prologue(
    ...
    void** out_emitter)
{
    woort_JIT_Asmjit_x64_emitter* const em =
        new (nothrow) woort_JIT_Asmjit_x64_emitter(cenv, ip);
...
{
    woort_JIT_Asmjit_x64_emitter* const em =
        static_cast<woort_JIT_Asmjit_x64_emitter*>(emitter);
...
void woort_JIT_Backend_x64_STORE(void* emitter, woort_Opcode_Global dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(lea(dst_ptr, em->static_slot(dst)));
...
void woort_JIT_Backend_x64_LOADPVALUE(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    const Gp ptr = em->load_stack_gp(src);
    em->store_stack(dst, qword_ptr(ptr));
}
...
void woort_JIT_Backend_x64_PUSHCCHK(void* emitter, woort_Opcode_Global src)
{
    ...
    else
    {
        ...
        if (is_constant)
            ...
            WOORT_JIT_CODE(mov(val, em->static_slot(src)));
        WOORT_JIT_CODE(mov(qword_ptr(em->m_sp), val));
    }
...
void woort_JIT_Backend_x64_POPS(void* emitter, woort_Opcode_Stack dst)
{
    ...
    em->store_stack(dst, qword_ptr(em->m_sp));
}
...
void woort_JIT_Backend_x64_CASTSFROM(void* emitter, woort_Opcode_Stack dst, woort_BoxValueType srctype, woort_Opcode_Stack src)
{
    ...
    case WOORT_BOX_VALUE_TYPE_VEC:
    {
        ...
        const Label L_ok = em->c->new_label();
        WOORT_JIT_CODE(test(dst_val, dst_val));
...
void woort_JIT_Backend_x64_CASTDYN(void* emitter, woort_Opcode_Stack dst, woort_BoxValueType target, woort_Opcode_Stack src)
{
    ...
    case WOORT_BOX_VALUE_TYPE_INT:
    {
        ...
        { em->store_stack(dst, Imm(0)); WOORT_JIT_CODE(jmp(L_done)); }
    ...
    case WOORT_BOX_VALUE_TYPE_STRING:
    {
        ...
        WOORT_JIT_CODE(bind(L_str));
        { em->store_stack(dst, val); WOORT_JIT_CODE(jmp(L_done)); }
        ...
        WOORT_JIT_CODE(bind(L_vec));
        {
            ...
            invoke_node->set_ret(0, result);
            const Label L_ok = em->c->new_label();
            WOORT_JIT_CODE(test(result, result));
            WOORT_JIT_CODE(jnz(L_ok));
            em->emit_failed_fallback(*em->m_ip);
            WOORT_JIT_CODE(bind(L_ok));
            em->store_stack(dst, result);
...
static void woort_JIT_x64_emit_closure_call(
    woort_JIT_Asmjit_x64_emitter* em, const Gp& target_closure)
{
    static_assert(sizeof(woort_Value) == 8, "");
    ...
    WOORT_JIT_CODE(mov(fn_ptr, qword_ptr(target_closure, off_fn)));

    const Gp script_fn = em->c->new_gp_ptr();
    WOORT_JIT_CODE(mov(script_fn, qword_ptr(target_closure, off_script_fn)));

    {
        const Label L_inline = em->c->new_label();

        WOORT_JIT_CODE(test(script_fn, script_fn));
        WOORT_JIT_CODE(jz(L_inline));

        WOORT_JIT_CODE(test(fn_ptr, fn_ptr));
        WOORT_JIT_CODE(jnz(L_inline));
    ...
    const Gp size_bytes = em->c->new_gp64();
    {
        ...
        WOORT_JIT_CODE(mov(size_count, qword_ptr(target_closure, off_size)));
        WOORT_JIT_CODE(mov(size_bytes, size_count));
        WOORT_JIT_CODE(shl(size_bytes, 3));
    }
    ...
    WOORT_JIT_CODE(sub(new_sb, Imm(static_cast<int32_t>(2 * sizeof(woort_Value)))));

    const Gp new_sp = em->c->new_gp_ptr();
    WOORT_JIT_CODE(mov(new_sp, new_sb));
    WOORT_JIT_CODE(sub(new_sp, size_bytes));
...

    {
        ...
        WOORT_JIT_CODE(test(size_bytes, size_bytes));
        WOORT_JIT_CODE(jz(L_no_datas));

        const Gp dst = em->c->new_gp_ptr();
        WOORT_JIT_CODE(lea(dst, ptr(new_sp, static_cast<int32_t>(sizeof(woort_Value)))));

        const Gp src = em->c->new_gp_ptr();
        WOORT_JIT_CODE(lea(src, ptr(target_closure, off_datas)));

        InvokeNode* invoke_node;
        WOORT_JIT_CODE(invoke(
            Out(invoke_node),
            Imm(reinterpret_cast<intptr_t>(WOORT_JIT_MEMCPY)),
...

    {
        ...
        WOORT_JIT_CODE(sub(bp_offset, em->m_sp));
        WOORT_JIT_CODE(shr(bp_offset, 3));
        WOORT_JIT_CODE(mov(dword_ptr(new_sb, bp_off), bp_offset.r32()));

        const Gp ret_addr = em->c->new_gp64();
        WOORT_JIT_CODE(mov(ret_addr, Imm(reinterpret_cast<intptr_t>(*em->m_ip + 1))));
        WOORT_JIT_CODE(mov(qword_ptr(new_sb, addr_off), ret_addr));
    }

    {
        ...
        WOORT_JIT_CODE(mov(dword_ptr(new_sb, way_off),
            Imm(static_cast<int32_t>(WOORT_CALL_WAY_FAR))));
...
void woort_JIT_Backend_x64_RETVC(void* emitter, woort_Opcode_Global src)
{
    ...
    else
    {
        ...
        if (is_constant)
            ...
            WOORT_JIT_CODE(mov(ret_val, em->static_slot(src)));
        WOORT_JIT_CODE(mov(return_place, ret_val));
    }
...
void woort_JIT_Backend_x64_JFWDNZ(void* emitter, woort_Opcode_Stack cond, woort_Opcode_CodeDiff off)
{
    ...
    WOORT_JIT_CODE(cmp(em->sb_slot(cond), Imm(0)));
...
void woort_JIT_Backend_x64_ADDI(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack a, woort_Opcode_Stack b)
{
    ...
    WOORT_JIT_CODE(add(result, em->sb_slot(b)));
...
void woort_JIT_Backend_x64_SUBI(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack a, woort_Opcode_Stack b)
{
    ...
    WOORT_JIT_CODE(sub(result, em->sb_slot(b)));
...
void woort_JIT_Backend_x64_MULI(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack a, woort_Opcode_Stack b)
{
    ...
    WOORT_JIT_CODE(imul(result, em->sb_slot(b)));
...
void woort_JIT_Backend_x64_NEGI(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(mov(result, em->sb_slot(src)));
...
void woort_JIT_Backend_x64_ADDR(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack a, woort_Opcode_Stack b)
{
    ...
    WOORT_JIT_CODE(movq(xmm, em->sb_slot(a)));
    WOORT_JIT_CODE(addsd(xmm, em->sb_slot(b)));
    em->store_stack(dst, xmm);
}
...
void woort_JIT_Backend_x64_SUBR(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack a, woort_Opcode_Stack b)
{
    ...
    WOORT_JIT_CODE(movq(xmm, em->sb_slot(a)));
    WOORT_JIT_CODE(subsd(xmm, em->sb_slot(b)));
    em->store_stack(dst, xmm);
}
...
void woort_JIT_Backend_x64_MULR(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack a, woort_Opcode_Stack b)
{
    ...
    WOORT_JIT_CODE(movq(xmm, em->sb_slot(a)));
    WOORT_JIT_CODE(mulsd(xmm, em->sb_slot(b)));
    em->store_stack(dst, xmm);
}
...
void woort_JIT_Backend_x64_DIVR(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack a, woort_Opcode_Stack b)
{
    ...
    WOORT_JIT_CODE(movq(xmm, em->sb_slot(a)));
    WOORT_JIT_CODE(divsd(xmm, em->sb_slot(b)));
    em->store_stack(dst, xmm);
}
...
void woort_JIT_Backend_x64_CADDI(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(add(reg_dst, em->sb_slot(src)));
...
void woort_JIT_Backend_x64_CSUBI(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(sub(reg_dst, em->sb_slot(src)));
    em->store_stack(dst, reg_dst);
}
...
void woort_JIT_Backend_x64_CMULI(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(imul(reg_dst, em->sb_slot(src)));
    em->store_stack(dst, reg_dst);
}
...
void woort_JIT_Backend_x64_CADDR(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(movq(xmm, em->sb_slot(dst)));
    WOORT_JIT_CODE(addsd(xmm, em->sb_slot(src)));
    em->store_stack(dst, xmm);
}
...
void woort_JIT_Backend_x64_CSUBR(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(movq(xmm, em->sb_slot(dst)));
    WOORT_JIT_CODE(subsd(xmm, em->sb_slot(src)));
    em->store_stack(dst, xmm);
}
...
void woort_JIT_Backend_x64_CMULR(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(movq(xmm, em->sb_slot(dst)));
    WOORT_JIT_CODE(mulsd(xmm, em->sb_slot(src)));
    em->store_stack(dst, xmm);
}
...
void woort_JIT_Backend_x64_CDIVR(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(movq(xmm, em->sb_slot(dst)));
    WOORT_JIT_CODE(divsd(xmm, em->sb_slot(src)));
    em->store_stack(dst, xmm);
}
...
void woort_JIT_Backend_x64_CMODR(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(movq(xmm_dst, em->sb_slot(dst)));
    WOORT_JIT_CODE(movq(xmm_src, em->sb_slot(src)));
...
void woort_JIT_Backend_x64_UNPACKVEC(void* emitter, woort_Opcode_Count n, woort_Opcode_Stack vec)
{
    ...
    WOORT_JIT_CODE(mov(vec_len, qword_ptr(gcvec_ptr, static_cast<int32_t>(offsetof(woort_GCVec, m_length)))));
    ...
    if (n != 0)
    {
        ...
        const Gp new_sp = em->c->new_gp_ptr();
        WOORT_JIT_CODE(mov(new_sp, em->m_sp));
        WOORT_JIT_CODE(sub(new_sp, Imm(static_cast<int32_t>(n * static_cast<int32_t>(sizeof(woort_Value))))));

        const Label L_sok = em->c->new_label();
        WOORT_JIT_CODE(cmp(new_sp, em->m_stack));
        WOORT_JIT_CODE(jae(L_sok));
        ...
        WOORT_JIT_CODE(mov(datas_ptr, qword_ptr(gcvec_ptr, static_cast<int32_t>(offsetof(woort_GCVec, m_datas)))));
...
void woort_JIT_Backend_x64_UNPACKVECX(void* emitter, woort_Opcode_Count n, woort_Opcode_Stack vec)
{
    ...
    if (n != 0)
    {
        ...
        {
            const Gp val = em->c->new_gp64();
            WOORT_JIT_CODE(mov(val, qword_ptr(datas_ptr, static_cast<int32_t>(i * static_cast<int32_t>(sizeof(woort_DynBox))))));
            WOORT_JIT_CODE(mov(qword_ptr(em->m_sp, static_cast<int32_t>((i + 1) * static_cast<int32_t>(sizeof(woort_Value)))), val));
        }
    }
}
...
void woort_JIT_Backend_x64_UNPACKVECALL(void* emitter, woort_Opcode_Stack count_dst, woort_Opcode_Count n, woort_Opcode_Stack vec)
{
    ...
    WOORT_JIT_CODE(shl(byte_count, 3));

    const Gp new_sp = em->c->new_gp_ptr();
    WOORT_JIT_CODE(mov(new_sp, em->m_sp));
    WOORT_JIT_CODE(sub(new_sp, byte_count));
    ...
    WOORT_JIT_CODE(sub(remaining, Imm(static_cast<int32_t>(n))));

    const Gp rem_bytes = em->c->new_gp64();
    WOORT_JIT_CODE(mov(rem_bytes, remaining));
    WOORT_JIT_CODE(shl(rem_bytes, 3));

    const Gp rem_src = em->c->new_gp_ptr();
    WOORT_JIT_CODE(lea(rem_src, ptr(datas_ptr, static_cast<int32_t>(n * static_cast<int32_t>(sizeof(woort_DynBox))))));

    const Gp rem_dst = em->c->new_gp_ptr();
    WOORT_JIT_CODE(lea(rem_dst, ptr(em->m_sp, static_cast<int32_t>((n + 1) * static_cast<int32_t>(sizeof(woort_Value))))));
...
void woort_JIT_Backend_x64_UNPACKVECXALL(void* emitter, woort_Opcode_Stack count_dst, woort_Opcode_Count n, woort_Opcode_Stack vec)
{
    ...
    const Gp dst_ptr = em->c->new_gp_ptr();
    WOORT_JIT_CODE(lea(dst_ptr, ptr(em->m_sp, static_cast<int32_t>(sizeof(woort_Value)))));
...
void woort_JIT_Backend_x64_PACKARG(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Count skip)
{
    ...
    WOORT_JIT_CODE(sub(pack_argc, Imm(static_cast<int32_t>(skip))));
    ...
    const Gp gcvec = em->c->new_gp_ptr();
    {
        ...
        WOORT_JIT_CODE(invoke(
            Out(invoke_node),
            Imm(reinterpret_cast<intptr_t>(woort_GCVec_new)),
...

    {
        ...
        WOORT_JIT_CODE(invoke(
            Out(invoke_node),
            Imm(reinterpret_cast<intptr_t>(_woort_GCVec_extern)),
    ...
    const Gp datas_ptr = em->c->new_gp_ptr();
    WOORT_JIT_CODE(mov(datas_ptr, qword_ptr(gcvec, static_cast<int32_t>(offsetof(woort_GCVec, m_datas)))));

    const Gp src_ptr = em->c->new_gp_ptr();
    WOORT_JIT_CODE(lea(src_ptr, ptr(em->m_sb, static_cast<int32_t>((4 + skip) * static_cast<int32_t>(sizeof(woort_Value))))));
...
void woort_JIT_Backend_x64_ASTORE(void* emitter, woort_Opcode_Global storage, woort_Opcode_Stack src)
{
    ...
    WOORT_JIT_CODE(mov(em->static_slot(storage), val));
}
...
void woort_JIT_Backend_x64_ALOAD(void* emitter, woort_Opcode_Stack dst, woort_Opcode_Global storage)
{
    ...
    em->store_stack(dst, em->static_slot(storage));
}
...
void woort_JIT_Backend_x64_CAS(void* emitter, woort_Opcode_Global storage, woort_Opcode_Stack desired, woort_Opcode_Stack expected)
{
    ...
    WOORT_JIT_CODE(lock().cmpxchg(em->static_slot(storage), desired_val, acc));
...
void woort_JIT_Backend_x64_JIFINITED(void* emitter, woort_Opcode_Global flag, woort_Opcode_CodeAbs target)
{
    ...
    WOORT_JIT_CODE(lea(storage_ptr, em->static_slot(flag)));
