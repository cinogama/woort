...
{
    if (woort_mem_gc_marking_state_flag)
    {
        woort_mem_mark_fuzzy_unit(src_unit);
        woort_mem_mark_fuzzy_unit(*modified_unit_addr);
    }
    ...
    void** modified_unit_addr, void* src_unit)
{
    if (woort_mem_gc_marking_state_flag)
    {
        woort_mem_mark_unit_head(src_unit);
        woort_mem_mark_fuzzy_unit_head(*modified_unit_addr);
    }
    *modified_unit_addr = src_unit;
}
    ...
    woort_Value* modified_value, woort_Value src_value)
{
    if (woort_mem_gc_marking_state_flag)
    {
        woort_mem_mark_fuzzy_unit(src_value.m_gcinstance);
        woort_mem_mark_fuzzy_unit(modified_value->m_gcinstance);
    }
    ...
    woort_DynBox* modified_box, woort_DynBox src_box)
{
    if (woort_mem_gc_marking_state_flag)
    {
        ...
        {
            woort_mem_mark_unit_head(_woort_boxed_to_gcunit(src_box.m_boxed));
        }
        if (modified_box->m_boxed != 0
            && 0 == (modified_box->m_boxed & 0b0111))
        {
            woort_mem_mark_fuzzy_unit_head(_woort_boxed_to_gcunit(modified_box->m_boxed));
        }
    }
...
{
    if (woort_mem_gc_marking_state_flag)
        woort_mem_mark_fuzzy_unit(addr);
}
...
{
    if (woort_mem_gc_marking_state_flag)
        woort_mem_mark_fuzzy_unit_head(unit);
}
...
{
    if (woort_mem_gc_marking_state_flag)
        woort_mem_mark_fuzzy_unit(value.m_gcinstance);
}
    ...
    woort_DynBox box)
{
    ...
    {
        woort_mem_mark_fuzzy_unit_head(_woort_boxed_to_gcunit(box.m_boxed));
    }
}
