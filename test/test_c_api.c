...
static void test_ir_vreg_and_argument(void)
{
    ...
    woort_IRFunction* f;
    TEST_ASSERT(woort_IRCompiler_add_function(irc, 3, 0, &f));
...
static void test_ir_label_and_bind(void)
{
    ...
    woort_CodeEnv_set_const_int(cenv, c0, 42);
...
static void test_ir_srcloc_push_pop(void)
{
    ...
    const char* path = woort_IRCompiler_intern_string(irc, "test/example.wo");
...
static void test_ir_stack_ops(void)
{
    ...
    {
        const woort_IRValue* v1 = woort_IRFunction_fetch_const(f_main, c1);
        const woort_IRValue* v2 = woort_IRFunction_fetch_const(f_main, c2);
...
static void test_ir_type_conversions(void)
{
    ...
    const woort_IRValue* v_int = woort_IRFunction_fetch_const(f, c_int_val);
    const woort_IRValue* v_real = woort_IRFunction_fetch_const(f, c_real_val);
    woort_IRValue* r1 = woort_IRFunction_new_vreg(f);
    woort_IRValue* r2 = woort_IRFunction_new_vreg(f);
    ...
    woort_CodeEnv_query_function(cenv, f, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_int(cenv, c_int_val, 42);
    woort_CodeEnv_set_const_real(cenv, c_real_val, 9.87);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_ir_real_arithmetic(void)
{
    ...
    const woort_IRValue* a = woort_IRFunction_fetch_const(f, c_a);
    const woort_IRValue* b = woort_IRFunction_fetch_const(f, c_b);
    ...
    woort_CodeEnv_query_function(cenv, f, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_real(cenv, c_a, 10.0);
    woort_CodeEnv_set_const_real(cenv, c_b, 3.0);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_ir_real_comparisons(void)
{
    ...
    woort_CodeEnv_query_function(cenv, f, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_real(cenv, c_a, 5.0);
    woort_CodeEnv_set_const_real(cenv, c_b, 3.0);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_ir_logical_ops(void)
{
    ...
    const woort_IRValue* t = woort_IRFunction_fetch_const(f, c_t);
    const woort_IRValue* fv = woort_IRFunction_fetch_const(f, c_f);
    ...
    woort_CodeEnv_query_function(cenv, f, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_int(cenv, c_t, 1);
    woort_CodeEnv_set_const_int(cenv, c_f, 0);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_ir_jcc_jccz(void)
{
    ...
    const woort_IRValue* x = woort_IRFunction_fetch_const(f, c_x);
    ...
    TEST_ASSERT(woort_IR_LTI(f, cond, x, woort_IRFunction_fetch_const(f, woort_IRCompiler_add_constant(irc))));

    const woort_IRValue* zero = woort_IRFunction_fetch_const(f, c_x);
    ...
    woort_CodeEnv_query_function(cenv, f, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_int(cenv, c_x, 0);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_ir_jcc_variants(void)
{
    ...
    woort_CodeEnv_query_function(cenv, f, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_int(cenv, c_a, 5);
    woort_CodeEnv_set_const_int(cenv, c_b, 3);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_vm_call_status(void)
{
    ...
    {
        woort_IRValue* r = woort_IRFunction_new_vreg(f_main);
        TEST_ASSERT(woort_IR_PUSHCHK(f_main, woort_IRFunction_fetch_const(f_main, c_arg)));
    ...
    const woort_Bytecode* entry_addr;
    woort_CodeEnv_query_function(cenv, f_main, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_int(cenv, c_arg, 0);
    woort_CodeEnv_set_const_extern_function(cenv, cfn, return_int_fn);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_codeenv_query_and_lock(void)
{
    ...
    woort_IRFunction* f1;
    TEST_ASSERT(woort_IRCompiler_add_function(irc, 0, 0, &f1));
    {
        woort_IRValue* v = woort_IRFunction_new_vreg(f1);
        TEST_ASSERT(v != NULL);
        TEST_ASSERT(woort_IR_ret(f1, woort_IRFunction_fetch_const(f1, woort_IRCompiler_add_constant(irc))));
    }

    woort_IRFunction* f2;
    TEST_ASSERT(woort_IRCompiler_add_function(irc, 0, 0, &f2));
    {
        woort_IRValue* v = woort_IRFunction_new_vreg(f2);
        TEST_ASSERT(v != NULL);
        (void)woort_IR_ret(f2, woort_IRFunction_fetch_const(f2, woort_IRCompiler_add_constant(irc)));
    }
...
static void test_box_types(void)
{
    ...
    const woort_IRValue* vi = woort_IRFunction_fetch_const(f, ci);
    const woort_IRValue* vr = woort_IRFunction_fetch_const(f, cr);
...
static void test_call_conventions(void)
{
    ...
    {
        woort_IRValue* r = woort_IRFunction_new_vreg(f_main);
        TEST_ASSERT(woort_IR_PUSHCHK(f_main, woort_IRFunction_fetch_const(f_main, c1)));
        TEST_ASSERT(woort_IR_PUSHCHK(f_main, woort_IRFunction_fetch_const(f_main, c2)));
        TEST_ASSERT(woort_IR_PUSHCHK(f_main, woort_IRFunction_fetch_const(f_main, c3)));
    ...
    woort_CodeEnv_set_const_extern_function(cenv, cfn, sumThree_fn);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_float_type(void)
{
    ...
    woort_CodeEnv_query_function(cenv, f, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_int(cenv, cr, 42);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_return_macros(void)
{
    ...
    TEST_ASSERT(woort_IRCompiler_add_function(irc, 0, 0, &f_main));
    {
        ...
        (void)woort_IR_PUSHCHK(f_main, woort_IRFunction_fetch_const(f_main, woort_IRCompiler_add_constant(irc)));
    ...
    woort_CodeEnv_query_function(cenv, f_main, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_extern_function(cenv, cfn_int, return_int_fn);
    woort_CodeEnv_set_const_extern_function(cenv, cfn_real, return_real_fn);
    woort_CodeEnv_set_const_extern_function(cenv, cfn_bool, return_bool_fn);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_vm_status_resync(void)
{
    ...
    woort_IRConstantIndex c_int = woort_IRCompiler_add_constant(irc);
    (void)woort_IR_ret(f, woort_IRFunction_fetch_const(f, c_int));
...
static void test_abort_and_yield(void)
{
    ...
    woort_CodeEnv_query_function(cenv, f_main, &entry_addr);
    woort_CodeEnv_lock(cenv);
    woort_CodeEnv_set_const_int(cenv, c_arg, 999);
    woort_CodeEnv_set_const_extern_function(cenv, cfn_abort, capture_int_fn);
    woort_CodeEnv_set_const_script_closure(cenv, c_entry, entry_addr);
    woort_CodeEnv_unlock(cenv);
...
static void test_struct_detailed_ops(void)
{
    ...
    const woort_IRValue* v_key1 = woort_IRFunction_fetch_const(f, c_key1);
    const woort_IRValue* v_val1 = woort_IRFunction_fetch_const(f, c_val1);
    const woort_IRValue* v_key2 = woort_IRFunction_fetch_const(f, c_key2);
    const woort_IRValue* v_val2 = woort_IRFunction_fetch_const(f, c_val2);
